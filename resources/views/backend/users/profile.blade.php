...
<div class="card shadow mb-4">
   ...
   <div class="card-header py-3">
     <h4 class="font-weight-bold">پروفایل</h4>
     <ul class="breadcrumbs">
         <li><a href="{{route('admin')}}" style="color:#999">داشبورد</a></li>
         <li><a href="" class="active text-primary">صفحه پروفایل</a></li>
     </ul>
   </div>
   <div class="card-body">
        <div class="row">
            <div class="col-md-4">
                <div class="card">
                    <div class="image">
                        @if($profile->photo)
                        <img class="card-img-top img-fluid rounded-circle mt-4" style="border-radius:50%;height:80px;width:80px;margin:auto;" src="{{$profile->photo}}" alt="عکس پروفایل">
                        @else 
                        <img class="card-img-top img-fluid rounded-circle mt-4" style="border-radius:50%;height:80px;width:80px;margin:auto;" src="{{asset('backend/img/avatar.png')}}" alt="عکس پروفایل">
                        @endif
                    </div>
            ...
            <div class="col-md-8">
                <form class="border px-4 pt-2 pb-3" method="POST" action="{{route('profile-update',$profile->id)}}">
                    ...
                    <div class="form-group">
                      ...
                      <input id="inputTitle" type="text" name="name" placeholder="نام را وارد کنید"  value="{{$profile->name}}" class="form-control">
                      @error('name')
                      ...
                      <div class="form-group">
                        ...
                        <input id="inputEmail" disabled type="email" name="email" placeholder="ایمیل را وارد کنید"  value="{{$profile->email}}" class="form-control">
                        @error('email')
                        <span class="text-danger">{{$message}}</span>
                        @enderror
                      </div>
                      ...
                      <div class="form-group">
                          ...
                          <select name="role" class="form-control">
                              <option value="">-----انتخاب نقش-----</option>
                                  ...
                                  <option value="user" {{(($profile->role=='user')? 'selected' : '')}}>کاربر</option>
                          </select>
                        @error('role')
                        <span class="text-danger">{{$message}}</span>
                        @enderror
                        ...
                        <button type="submit" class="btn btn-success btn-sm">به‌روزرسانی</button>
                </form>
            </div>
        </div>
   </div>
